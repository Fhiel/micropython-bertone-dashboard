...
**1983 BERTONE X1/9e ***electric-speed*** – fully converted to electric drive**  
Triple 128×32 SSD1306 OLED dashboard with custom pixel-perfect 12x16 and 16×21 font  
...
Dependencies: Imports the compiled font data (font\_<size>\_data.py).
...
Core Task: Reads the raw pixel arrays, applies the VLSB conversion rule (packing vertical pixel columns into bytes), and saves the resulting, optimized binary data into static Python files (font\_[size]\_data.py).

## 5. font\_[size]\_data.py - The Data Artifacts
...
## **Adding New Characters (12x16 and 16x21)**

The file raw\_font.py serves as the source for both the 12x16 (small) and 16x21 (large) font definitions. Inside, you will find all existing characters defined using binary patterns (sequences of 0s and 1s) which represent the pixel data.

To add a new symbol, follow these steps:

1. **Copy and Paste:** Copy an existing character definition block.  
2. **Define Alias:** Assign **it** a new, unique alias (variable name) that is not yet in use.  
3. **Modify Pattern:** Modify the 0 and 1 pattern to form your desired symbol.  
4. **Verify Dimensions:** Ensure that the total number of rows and columns (the dimensions of the binary block) remains consistent with the respective font size (12x16 or 16x21).

After modifying raw\_font.py:

1. **Generate Files:** Save the modified raw\_font.py. Run font\_converter.py in the same directory to generate the new configuration files: font\_12x16\_data.py and font\_16x21\_data.py.  
2. **Deployment:** Copy **these files** to your target device.

You are now ready to use your custom letter or number in your application\!
___
MIT License • Made with love for the electric BERTONE X1/9e
Special thanks to Gemini (AI) – without that this would still be 8×8 mouse cinema.
